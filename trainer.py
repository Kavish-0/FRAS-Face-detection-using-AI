...
def train_model():
    ...
    if not os.path.exists(FACE_DIR):
        logger.error("Face dataset folder not found")
        return
    ...
    for filename in os.listdir(FACE_DIR):
        ...
        img_numpy = cv2.resize(img_numpy, FACE_SIZE)  # must match recognition size
    ...
    logger.info(f"Training complete. Total images: {len(face_samples)}")
